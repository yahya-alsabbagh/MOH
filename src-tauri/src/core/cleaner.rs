...
/// التطبيع الموحّد لعناوين أعمدة الإكسل.
///
/// **مصدر الحقيقة الوحيد** — تُستدعى من `read_headers` ومن كل مسارات الاستيراد،
/// والواجهة لا تطبّع شيئاً. أي اختلاف بين الطرفين كان يجعل المستخدم يختار عموداً
/// من القائمة ثم يفشل الرفع بخطأ «العمود غير موجود»، ويلوّث سجل الأعمدة بنسخ مكرّرة.
pub fn normalize_header(raw: &str) -> String {
    // يطوي كل تتابع فراغات (بما فيه \n و \t) إلى مسافة واحدة، ويحذف الأطراف.
    raw.split_whitespace().collect::<Vec<_>>().join(" ")
}

/// خط أنابيب توحيد الأسماء العربية (Arabic Name Normalization Pipeline)
/// يُستخدم لتوحيد الأسماء قبل مقارنتها للكشف عن التكرارات.
pub fn normalize_arabic_name(text: &str) -> String {
    // ═══════════════════════════════════════════════════════════
    // المرحلة 1: إزالة التشكيل والتطويل (Filtering)
    // ═══════════════════════════════════════════════════════════
    let filtered: String = text
        .chars()
        .filter(|c| {
            let u = *c as u32;
            // إزالة الحركات العربية (الفتحة، الضمة، الكسرة، التنوين، الشدة، السكون)
            !(0x064B..=0x0652).contains(&u)
            // إزالة كشيدة التطويل
            && u != 0x0640
        })
        .collect();
    ...
    // ═══════════════════════════════════════════════════════════
    // المرحلة 2: توحيد الحروف (Character Unification)
    // ═══════════════════════════════════════════════════════════
    let unified: String = filtered
        .chars()
        .map(|c| match c {
    ...
    // ═══════════════════════════════════════════════════════════
    // المرحلة 3: توحيد الأسماء المركبة (Compound Names)
    // ═══════════════════════════════════════════════════════════
    // أولاً: توحيد الفراغات قبل معالجة الأسماء المركبة
    let spaced: String = unified.split_whitespace().collect::<Vec<_>>().join(" ");

    // سوابق الأسماء المركبة: "عبد الله" -> "عبدالله"
    let merged = spaced
        // --- عبد + ... ---
        .replace("عبد ", "عبد")
        // --- ابو/ابي + ... ---
        .replace("ابو ", "ابو")
        // --- ال التعريف الملتصقة بالاسم السابق ---
        .replace(" ال ", "ال");

    // ═══════════════════════════════════════════════════════════
    // المرحلة 4: معالجة المسافات النهائية (Whitespace)
    // ═══════════════════════════════════════════════════════════
